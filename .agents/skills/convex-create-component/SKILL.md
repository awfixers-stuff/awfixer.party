...
10. Run `bunx --bun convex dev` and fix codegen, type, or boundary issues before finishing.
...
Unless the user explicitly wants an bun --bun package, default to a local component:
...
- Let `bunx --bun convex dev` generate the component's own `_generated/` files
...
1. `bunx --bun convex codegen --component-dir convex/components/<name>`
2. `bunx --bun convex codegen`
3. `bunx --bun convex dev`
...
- [ ] Ran `bunx --bun convex dev` and fixed codegen or type issues
