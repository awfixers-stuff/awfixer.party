...
Read this file when the user wants a reusable bun --bun package or a component shared across multiple apps.
...
- Prefer starting from `bunx --bun create-convex@latest --component` when possible
...
1. `bunx --bun convex codegen --component-dir ./path/to/component`
2. Run the package build command
3. Run `bunx --bun convex dev --typecheck-components` in the example app

Do not assume normal app codegen is enough for packaged component workflows.

## Package Exports

If publishing to bun --bun, make sure the package exposes the entry points apps need:
