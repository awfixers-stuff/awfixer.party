...
```bash
bun --bun install @convex-dev/migrations
```
...
bunx --bun convex run migrations:runIt

# Or use the general-purpose runner
bunx --bun convex run migrations:run '{"fn": "migrations:addDefaultRole"}'
```
...
```bash
bunx --bun convex run migrations:runAll
```
...
```bash
bunx --bun convex run migrations:runIt '{"dryRun": true}'
```
...
```bash
bunx --bun convex run --component migrations lib:getStatus --watch
```

## Cancel a Running Migration

```bash
bunx --bun convex run --component migrations lib:cancel '{"name": "migrations:addDefaultRole"}'
```
...
```bash
bunx --bun convex deploy --cmd 'bun --bun run build' && bunx --bun convex run migrations:runAll --prod
```
