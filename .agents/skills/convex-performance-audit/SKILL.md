...
- `bunx --bun convex insights --details` reports high bytes read, documents read, or OCC conflicts
...
2. If CLI insights are available, run `bunx --bun convex insights --details`. Use `--prod`, `--preview-name`, or `--deployment-name` when needed.
   - If the local repo's Convex CLI is too old to support `insights`, try `bunx --bun -y convex@latest insights --details` before giving up.
