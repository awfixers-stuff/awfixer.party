...
description: Initializes a new Convex project from scratch or adds Convex to an existing app. Use this skill when starting a new project with Convex, scaffolding with bun --bun create convex@latest, adding Convex to an existing React, Next.js, Vue, Svelte, or other frontend, wiring up ConvexProvider, configuring environment variables for the deployment URL, or running bunx --bun convex dev for the first time, even if the user just says "set up Convex" or "add a backend."
---
...
2. If new project, pick a template and scaffold with `bun --bun create convex@latest`
3. If existing app, install `convex` and wire up the provider
4. Run `bunx --bun convex dev` to connect a deployment and start the dev loop
...
```bash
bun --bun create convex@latest my-app -- -t owner/repo
bun --bun create convex@latest my-app -- -t owner/repo#branch
```
...
```bash
bun --bun create convex@latest my-app -- -t react-vite-shadcn
cd my-app
bun --bun install
```

The scaffolding tool creates files but does not run `bun --bun install`, so you must run it yourself.

To scaffold in the current directory (if it is empty):

```bash
bun --bun create convex@latest . -- -t react-vite-shadcn
bun --bun install
```

### Start the dev loop

`bunx --bun convex dev` is a long-running watcher process that syncs backend code to a Convex deployment on every save. It also requires authentication on first run (browser-based OAuth). Both of these make it unsuitable for an agent to run directly.

**Ask the user to run this themselves:**

Tell the user to run `bunx --bun convex dev` in their terminal. On first run it will prompt them to log in or develop anonymously. Once running, it will:
...
The user should keep `bunx --bun convex dev` running in the background while you work on code. The watcher will automatically pick up any files you create or edit in `convex/`.
...
```bash
bun --bun install convex
```

### Initialize and start dev loop

Ask the user to run `bunx --bun convex dev` in their terminal. This handles login, creates the `convex/` directory, writes the deployment URL to `.env.local`, and starts the file watcher. See the notes in Path 1 about why the agent should not run this directly.
...
`bunx --bun convex dev` writes the correct variable to `.env.local` automatically.
...
```bash
CONVEX_AGENT_MODE=anonymous bunx --bun convex dev
```
...
1. The user confirms `bunx --bun convex dev` is running without errors
...
Always use `bunx --bun convex dev` during development. It runs against your personal dev deployment and syncs code on save.
...
- [ ] If new project: scaffolded with `bun --bun create convex@latest` using appropriate template
- [ ] If existing app: installed `convex` and wired up the provider
- [ ] User has `bunx --bun convex dev` running and connected to a deployment
