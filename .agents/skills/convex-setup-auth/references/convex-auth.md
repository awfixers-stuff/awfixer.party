...
5. Make sure the project has a configured Convex deployment:
   - run `bunx --bun convex dev` first if `CONVEX_DEPLOYMENT` is not set
...
6. Install the auth packages:
   - `bun --bun install @convex-dev/auth @auth/core@0.37.0`
7. Run the initialization command:
   - `bunx --bun @convex-dev/auth`
...
12. Run `bunx --bun convex dev --once` or the normal dev flow to push the updated schema and generated code
...
- Treat `bunx --bun @convex-dev/auth` as a required initialization step for existing apps, not an optional extra

## Concrete Steps

1. Install `@convex-dev/auth` and `@auth/core@0.37.0`
2. Run `bunx --bun convex dev` if the project does not already have a configured deployment
3. If `bunx --bun convex dev` blocks on interactive setup, ask the user explicitly to finish configuring the Convex deployment
4. Run `bunx --bun @convex-dev/auth`
...
9. Run `bunx --bun convex dev --once` or the normal dev flow so the updated schema and auth files are pushed
...
- generated Convex Auth setup produced by `bunx --bun @convex-dev/auth`
- an existing configured Convex deployment, or the ability to create one with `bunx --bun convex dev`
...
- `bunx --bun @convex-dev/auth` is important because it initializes the auth setup, including the key material. Do not skip it when adding Convex Auth to an existing project.
- `bunx --bun @convex-dev/auth` will fail if the project does not already have a configured `CONVEX_DEPLOYMENT`.
- `bunx --bun convex dev` may require interactive setup for deployment creation or project selection. If that happens, ask the user explicitly for that human step instead of guessing.
- `bunx --bun @convex-dev/auth` does not finish the whole integration by itself. You still need to add `authTables`, swap in `ConvexAuthProvider`, and configure at least one auth method.
...
If `bunx --bun convex dev` or deployment setup requires human input:
...
- Run `bunx --bun convex dev --once` or the normal dev flow after setup changes and confirm Convex codegen and push succeed
...
- [ ] Run `bunx --bun convex dev` first if needed
- [ ] Run `bunx --bun @convex-dev/auth`
...
- [ ] Run `bunx --bun convex dev --once` or the normal dev flow after setup changes
