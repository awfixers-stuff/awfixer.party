...
//#region server/api/v1/toc.get.ts
var toc_get_default = defineHandler(async (event) => {
	...
	try {
		...
		const items = await getConvexClient().query(api.toc.getTocForPath, { path: normalized.replace(/\/+$/, "") || "/" });
...
//#endregion
export { toc_get_default as default };
