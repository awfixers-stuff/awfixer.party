...
// Static collab/AI tokens are for local development only. In production, use USE_JWT_TOKEN_API_ENDPOINT.
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'production') {
  if (TIPTAP_COLLAB_TOKEN || TIPTAP_AI_TOKEN) {
    console.error(
      '[Security] Static TipTap collaboration/AI tokens are set via NEXT_PUBLIC_ env vars. ' +
        'These tokens are exposed in the client bundle. Set NEXT_PUBLIC_USE_JWT_TOKEN_API_ENDPOINT ' +
        'and implement a server-side token endpoint instead.',
    );
  }
}
